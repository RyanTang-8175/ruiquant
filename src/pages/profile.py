...
def render_profile_page():
    ...
    with st.form("cfg"):
        st.markdown("**主模型（OpenAI 兼容）**")
        st.caption("可填 DeepSeek、通义千问 DashScope compatible-mode，或其他 OpenAI 兼容接口。")
        nk = st.text_input("API Key", value=cur_k, type="password", placeholder="sk-...")
        nu = st.text_input("Base URL", value=cur_u, placeholder="https://api.deepseek.com 或 https://dashscope.aliyuncs.com/compatible-mode/v1")
        nm = st.text_input("Model", value=cur_m, placeholder="deepseek-chat / qwen-plus")
        st.markdown("---")
        st.markdown("**备选模型（Mimo）**")
        st.caption("DeepSeek 不可用时自动切换；如果主模型是千问或自定义接口，也会在主模型失败时兜底。地址 https://token-plan-cn.xiaomimimo.com/anthropic，模型 mimo-v2.5-pro")
