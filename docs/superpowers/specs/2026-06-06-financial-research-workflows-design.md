...
- iFinD 调用预算和实际用量变化；
- 结构化研究草稿；
- `draft`、`blocked_missing_evidence` 或 `blocked_quota_overrun` 状态；
- 人工复核状态。
...
AI 可以列出并运行工作流，但没有签核工具。所有输出仍是草稿，不可自动标记为
人工通过。
...
- Playbook 明确最大预算；Runner 校验实际调用增量，超预算时状态变为
  `blocked_quota_overrun`，并禁止人工批准。
- 结果页面展示预算，不在页面刷新时自动运行。
...
- 必须有财报或业绩公告原文链接；只有标题元数据不能通过财报质量门。
...
- 即使用户点击通过，底层存储也会再次校验质量门；阻断状态不能被批准。
...
- 三套 JSON Playbook、Registry、Runner 和原子持久化；
- 研究页 SOP 标签、预算预览、来源账本、质量门、草稿和人工复核；
- DeepSeek 工作流工具与证据约束；
- 行情抓取时间写入来源账本；
- 超预算、缺财报原文和质量门失败的后端阻断；
- 缓存命中时重算摘要卡与质量，避免旧缓存继续展示错误计数；
- K 线摘要固定展示最新四根，缺日期时使用非空可访问标签；
- 不可用基础指标从“可用指标”计数和展示中排除。

最终验证：`78 passed`，Python 全量编译、JSON Playbook、`git diff --check`
和 HTTP 健康检查通过；本地 Streamlit 实测完成“研究页 -> 独立主题 SOP”以及
“600900 底稿 -> SOP -> 来源/质量门 -> 人工复核”完整链路。发布前还补上了
无效股票代码零调用、工作流运行记录并发锁，以及切换工作流时不串用旧结果。
SOP 复用输入底稿时新增 iFinD 调用为 0。
